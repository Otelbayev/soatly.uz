import type { Metadata } from 'next';
import Link from 'next/link';
import Image from 'next/image';
import { api } from '@/lib/api';
import { JsonLd } from '@/components/JsonLd';
import ProductCard from '@/components/ProductCard';
import { ChevronRight } from 'lucide-react';

const BASE_URL = 'https://soatly.uz';

interface Props { params: Promise<{ slug: string }> }

const truncate = (s: string, n: number) =>
  s.length > n ? s.slice(0, n - 1).trimEnd() + '…' : s;

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  try {
    const { slug } = await params;
    const c = await api.getCategoryBySlug(slug);
    const title = `${c.name_uz} soatlar — Premium kolleksiya`;
    const description = truncate(
      c.description_uz ||
        `${c.name_uz} kategoriyasidagi premium qo'l soatlari — SOATLY.uz. ` +
          "Sertifikatlangan asl mahsulot, O'zbekiston bo'ylab tezkor yetkazib berish.",
      200,
    );
    const url = `${BASE_URL}/categories/${slug}`;
    const images = c.icon
      ? [{ url: c.icon, alt: c.name_uz, width: 1200, height: 1200 }]
      : undefined;
    return {
      title,
      description,
      keywords: [c.name_uz, 'soat', 'qo\'l soati', `${c.name_uz} soatlar`, 'soatly.uz', 'luxury watches'],
      alternates: { canonical: url },
      openGraph: {
        title: `${title} | SOATLY`,
        description,
        url,
        siteName: 'SOATLY',
        type: 'website',
        locale: 'uz_UZ',
        images,
      },
      twitter: {
        card: 'summary_large_image',
        title: `${title} | SOATLY`,
        description,
        images: c.icon ? [c.icon] : undefined,
      },
      robots: { index: true, follow: true, googleBot: { index: true, follow: true, 'max-image-preview': 'large' } },
    };
  } catch {
    return { title: 'Kategoriya topilmadi', robots: { index: false, follow: false } };
  }
}

export const dynamic = 'force-dynamic';

export default async function CategorySlugPage({ params }: Props) {
  const { slug } = await params;
  const category = await api.getCategoryBySlug(slug).catch(() => null);

  if (!category) {
    return (
      <div className="min-h-screen pt-32 flex flex-col items-center justify-center gap-4 px-4">
        <h1 className="font-heading text-2xl font-bold" style={{ color: 'var(--text)' }}>Kategoriya topilmadi</h1>
        <Link href="/categories" className="gold-btn">Kategoriyalar</Link>
      </div>
    );
  }

  const productsData = await api
    .getProducts({ category: slug, limit: 24 })
    .catch(() => ({ products: [], total: 0, page: 1, pages: 1, limit: 24 }));
  const products = productsData.products;
  const url = `${BASE_URL}/categories/${slug}`;

  const breadcrumbLd = {
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Bosh sahifa', item: BASE_URL },
      { '@type': 'ListItem', position: 2, name: 'Kategoriyalar', item: `${BASE_URL}/categories` },
      { '@type': 'ListItem', position: 3, name: category.name_uz, item: url },
    ],
  };

  const collectionLd = {
    '@context': 'https://schema.org',
    '@type': 'CollectionPage',
    name: `${category.name_uz} soatlar`,
    description: category.description_uz || `${category.name_uz} kategoriyasidagi soatlar`,
    url,
    image: category.icon || undefined,
    isPartOf: { '@type': 'WebSite', name: 'SOATLY', url: BASE_URL },
    mainEntity: {
      '@type': 'ItemList',
      numberOfItems: productsData.total,
      itemListElement: products.slice(0, 20).map((p, i) => ({
        '@type': 'ListItem',
        position: i + 1,
        url: `${BASE_URL}/products/${p.id}`,
        name: p.name_uz,
        image: p.images?.[0],
      })),
    },
  };

  return (
    <>
      <JsonLd data={breadcrumbLd} />
      <JsonLd data={collectionLd} />

      <div className="min-h-screen pt-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
          <nav className="flex items-center gap-2 text-xs mb-6" style={{ color: 'var(--text-muted)' }}>
            <Link href="/" className="hover:text-gold-light transition-colors">Bosh sahifa</Link>
            <ChevronRight size={12} style={{ color: 'var(--border)' }} />
            <Link href="/categories" className="hover:text-gold-light transition-colors">Kategoriyalar</Link>
            <ChevronRight size={12} style={{ color: 'var(--border)' }} />
            <span style={{ color: 'var(--text)' }}>{category.name_uz}</span>
          </nav>

          <header className="flex items-center gap-5 mb-10">
            {category.icon && (
              <div className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-2xl overflow-hidden shrink-0"
                style={{ background: 'var(--bg-surface2)', border: '1px solid var(--border-gold)' }}>
                <Image src={category.icon} alt={category.name_uz} fill className="object-cover" sizes="96px" priority />
              </div>
            )}
            <div className="min-w-0">
              <p className="text-xs font-bold uppercase tracking-[0.2em] mb-1" style={{ color: 'var(--gold)' }}>
                Kategoriya
              </p>
              <h1 className="section-title mb-2" style={{ color: 'var(--text)' }}>{category.name_uz}</h1>
              {category.description_uz && (
                <p className="text-sm sm:text-base leading-relaxed max-w-2xl" style={{ color: 'var(--text-muted)' }}>
                  {category.description_uz}
                </p>
              )}
              <p className="text-xs mt-2" style={{ color: 'var(--text-subtle)' }}>
                {productsData.total} ta mahsulot
              </p>
            </div>
          </header>

          {products.length === 0 ? (
            <div className="text-center py-16">
              <p className="text-base mb-4" style={{ color: 'var(--text-muted)' }}>
                Ushbu kategoriyada hozircha mahsulot yo&apos;q
              </p>
              <Link href="/products" className="gold-btn">Boshqa soatlarni ko&apos;ring</Link>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                {products.map((p) => <ProductCard key={p.id} product={p} />)}
              </div>
              {productsData.total > products.length && (
                <div className="text-center mt-10">
                  <Link href={`/products?category=${slug}`} className="outline-btn">
                    Hamma {productsData.total} ta mahsulotni ko&apos;rsatish
                  </Link>
                </div>
              )}
            </>
          )}
        </div>
      </div>
    </>
  );
}
